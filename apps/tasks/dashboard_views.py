...
class TodayTasksView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        dashboard_data = TaskService.get_daily_dashboard(user=request.user, target_date=today)
...
class DailyLoadView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        dashboard_data = TaskService.get_daily_dashboard(user=request.user, target_date=target_date)
        serializer = DailyDashboardSerializer(dashboard_data)
        return Response(serializer.data, status=status.HTTP_200_OK)
